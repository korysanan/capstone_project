...
class CommunityBookmarkData {
  ...
  static List<String> getBookmarkNames() {
    return communities_bookmarks.map((bookmark) => bookmark['title'] as String).toList();
  }

  // 북마크 보유 여부 (true, false)
  static bool isBookmarked(int postId) {
    return communities_bookmarks.any((bookmark) => bookmark['id'] as int == postId);
  }

  // 북마크 삭제
  static void deleteBookmark(String community_Title) {
    int? postId;

    for (var bookmark in communities_bookmarks) {
      if (bookmark['title'] == community_Title) {
